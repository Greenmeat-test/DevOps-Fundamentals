...
if ! [ -e $1 ] || ! [ -f $1 ];      # check if if input variable exist and is it file
then
...
awk '{
    if ( index($0, "[")==1 ){                                                                 # first line
       nameSample = substr($0,index($0,"[") + 2,(index($0, "]")-(index($0,"[")+3)))
    ...
    if( NR>2 && NR<numberOfTests+3 ){
        ...
        if(NR==numberOfTests+2)
            printf ("}" "\n")     # if last number of test we did not wrote a comma
        else 
            printf ("}," "\n")
    }
}END{           # last line
        ...
        failed = substr($0, 
                        ...
                        match($0, /tests failed/) - match($0,/[0-9]+ tests failed/) - 1 )       # find test failed number
       
        rating = substr($0,
                        match($0, /rated as/) +9,\
                        match($0, /%/) - match($0, /rated/) - 9 )                        # find rating without % symbol
                        
        durationALL = substr($0, match($0,/spent/)+6, length($0) - 2)
